...
export const weatherApi = {
  ...
  getForecasts: async (): Promise<WeatherForecast[]> => {
    const response = await apiClient.get<WeatherForecast[]>('/api/weatherforecast')
    return response.data
  },
  // Secure endpoint - requires authentication
  getSecureForecasts: async (): Promise<SecureWeatherForecast[]> => {
    const response = await apiClient.get<SecureWeatherForecast[]>('/api/weatherforecast/secure')
    return response.data
  },
}
