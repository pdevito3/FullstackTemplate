...
Use value objects to encompass common domain concepts with behavior and validation.
...
public class Sex : ValueObject
{
    ...
    private  abstract class SexEnum(string name, int value) : SmartEnum<SexEnum>(name, value)
    {
        ...
        private class UnknownType() : SexEnum("Unknown", 0);
    
        private class MaleType() : SexEnum("Male", 1);
    
        private class FemaleType() : SexEnum("Female", 2);
        
        private class NotGivenType() : SexEnum("Not Given", 3);
    }
}
```
...
### 10. Use SmartEnum for option backed value objects

Implement value objects backed by smart enums when they have fixed options like status, sex, ethnicity, etc. 
Additionally, back them with behavior methods for state-dependent operations.
...
When adding a new property to an existing entity:

1. **Add with private setter:**
   ```csharp
   public string NewProperty { get; private set; }
   ```

2. **Update factory method if set at creation:**
   ```csharp
   public static Patient Create(PatientForCreation forCreation)
   {
       var patient = new Patient
       {
           // existing properties...
           NewProperty = forCreation.NewProperty,
       };
       // ...
   }
   ```

3. **Add to Update method if modifiable:**
   ```csharp
   public Patient Update(PatientForUpdate forUpdate)
   {
       // existing updates...
       NewProperty = forUpdate.NewProperty;
       QueueDomainEvent(new PatientUpdated { Id = Id });
       return this;
   }
   ```

4. **For value objects, use the factory pattern:**
   ```csharp
...
### Adding New Business Methods

When adding new business operations to an existing entity:

1. **Add domain method with guards:**
   ```csharp
   public Order Submit()
   {
       GuardIfNotDraft("Cannot submit order that is not in draft status");

       Status = OrderStatus.Submitted();
       SubmittedAt = DateTimeOffset.UtcNow;

       QueueDomainEvent(new OrderSubmitted { OrderId = Id });

       return this;
   }
   ```

2. **Add corresponding domain event:**
   ```csharp
...
3. **Add guard method if needed:**
   ```csharp
   private void GuardIfNotDraft(string message)
   {
       if (Status != OrderStatus.Draft())
           throw new ValidationException(nameof(Order), message);
   }
   ```

### Adding New Collections

When adding a new collection relationship:
...
{
    GuardIfInFinalState("Cannot add items");

    var lineItem = LineItem.Create(product, quantity, this);
...
// Business method to remove items
public void RemoveLineItem(LineItem lineItem)
{
    GuardIfInFinalState("Cannot remove items");
